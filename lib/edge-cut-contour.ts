...
/**
 * Radius of the round close that fillets the two joins between ring and
 * outline, as a fraction of the ring radius. Raised from 0.6, which left the
 * ring meeting the piece on a hard shoulder. A straight-sided taper was
 * tried first and reverted: stamped over an outline narrower than itself it
 * stuck out as two points, which the client rightly called fangs. A round
 * close cannot do that — it only ever fills a concave corner.
 */
